...
class MarketEventsSpidersSpiderMiddleware(object):
...
class MarketEventsSpidersDownloaderMiddleware(object):
  # Not all methods need to be defined. If a method is not defined,
  ...
  def process_request(self, request, spider):
    ...
    if self.per_proxy:
      ...
      if proxy not in self.proxy2ua:
        ...
        spider.logger.debug('Assign User-Agent %s to Proxy %s' % (self.proxy2ua[proxy],
                                                           proxy))
